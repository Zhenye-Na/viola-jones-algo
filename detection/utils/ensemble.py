...
def ensemble_vote(int_img, classifiers):
    ...
    :rtype: int
    """
    return 1 if sum([c._get_delta(int_img) for c in classifiers]) >= 0 else 0
...
def ensemble_vote_all(int_imgs, classifiers):
    ...
    return list(map(vote_partial, [img for img in int_imgs]))
...
def reconstruct(classifiers, img_size):
    ...
    #                 if y >= c.height / 2:
    #                     sign = (sign + 1) % 2
    #                 image[c.top_left[1] + y, c.top_left[0] +
    #                       x] += 1 * sign * c.weight
    ...
    #                 sign = (sign + 1) % 2
    #             for y in range(c.height):
    #                 image[c.top_left[0] + x, c.top_left[1] +
    #                       y] += 1 * sign * c.weight
